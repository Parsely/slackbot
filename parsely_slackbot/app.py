...
config_args = lambda x: None
config_args.config = 'config.yaml'

# app has to be global for Gunicorn / Heroku deploy
app = Flask(__name__)
...
@slack.command(config['slash_command'], token=config['slack_token'],
...
def parsely(text=None, channel=None, **kwargs):
    ...
    if not parsed_commands:
        return slack.response(
            "Sorry, I didn't understand that query! Maybe try /parsely help?")
